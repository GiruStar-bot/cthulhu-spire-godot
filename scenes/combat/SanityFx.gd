...
##  - 低い状態（SanityTiers の 3 段階）：パネルに絡みつく触手（SanityTendrils）、わずかな彩度低下＋持続音
...
## 触手の段階が変わった瞬間（段階を飛ばしても 1 回、一番上の段階）。AudioManager.play_sanity_tendril につなぐ。
signal tendril_changed(tier: int, duration: float, is_retract: bool)

## --- 美術素材（デザインくん / art/sanity-fx）。無ければ描かない（雫と端インクだけ代わりの描画あり）。触手は SanityTendrils.gd
...
func _ready() -> void:
	...
	tendril_changed.connect(_on_tendril_changed)
	_layout()
...
func _build() -> void:
	...
	_tendrils = SanityTendrils.new()
	add_child(_tendrils)
	_tendrils.changed.connect(func(tier: int, duration: float, is_retract: bool) -> void: tendril_changed.emit(tier, duration, is_retract))
...
func set_sanity(sanity: int, max_sanity: int) -> void:
	...
	if tier == _tier:
		return
	_tier = tier
	_tendrils.set_tier(tier)
	_apply_desat()


## 触手を付けるパネル：{"end_turn": Control, "log": Control, "hud": Control}
func set_tendril_panels(panels: Dictionary) -> void:
	_tendrils.set_panels(panels)


func tendrils() -> SanityTendrils:
	return _tendrils
...
func _apply_desat() -> void:
	var desat_amount: float = DESAT_BY_TIER[_tier]
	_desat.visible = desat_amount > 0.0
	_desat_mat.set_shader_parameter("amount", desat_amount)


func _on_tendril_changed(tier: int, duration: float, is_retract: bool) -> void:
	if AudioManager.has_method("play_sanity_tendril"):
		AudioManager.call("play_sanity_tendril", tier, duration, is_retract)
