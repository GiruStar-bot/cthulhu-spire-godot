...
const CARDS := {
	...
	"hundred_eyed_child": {
		...
		"text": "プレイ不可。",
		"upgradedText": "プレイ不可。",
		"flavor": "無数の赤い瞳が、こちらを見返している。",
		"art": "res://art/pixel/cards/hundred_eyed_child.jpg",
		"target": "none",
		"effects": [],
		"upgradedEffects": [],
	},
	## アイホートくん専用。HPに通ると GameState 側で呪い（eihort_curse_floor）が付く。
	"child_bearing": {
		"id": "child_bearing",
		"name": "子を宿す",
		"type": "attack",
	...
	"ultimate_arcane": {
		...
		"cost": 0, "sell_price": 20, "drop_weight": 9, "owner": "shared",
...
## cards.ts aiCardPool()。max_enemy_tier を渡すと、カードの enemy_tier（1=雑魚から使う、
## 2=エリート以上）がその値以下のものだけに絞る。enemy_tier の無いカードは、デッキ指定の敵（ボス）専用。
static func ai_card_pool(tag: String, max_enemy_tier = null, archetype = null) -> Array:
	var out: Array = []
	for c in CARDS.values():
		...
		if max_enemy_tier != null:
			...
			if tier <= 0 or tier > int(max_enemy_tier):
				continue
...
## drop_weight の無いカードの既定値（旧コモン相当）。新カード追加時に書き忘れても出なくならないように。
...
## store.ts の weightedCard(owner, rand)。rewardPool(owner) から _pick_weighted で1枚選ぶ。
static func weighted_card(owner: String, rand: Callable) -> Dictionary:
	var def: Dictionary = _pick_weighted(reward_pool(owner), rand)
